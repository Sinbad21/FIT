...
function Chart({ title, data, dataKey, color, unit }: { title: string; data: any[]; dataKey: string; color: string; unit?: string }) {
  return (
    <article className="glass-card rounded-[1.6rem] p-4">
      ...
      {data.length === 0 ? <p className="text-sm text-slate-500">Nessun dato ancora.</p> : (
...
export function ProgressCharts({ progress }: { progress: any }) {
  return (
    <div className="space-y-4">
      ...
      <Chart title="Calorie giornaliere" data={progress.nutrition || []} dataKey="calories" color="#0ea5e9" />
      ...
      <article className="glass-card rounded-[1.6rem] p-4">
        ...
        {(progress.workouts || []).length === 0 ? <p className="text-sm text-slate-500">Nessun allenamento registrato.</p> : (
          <div className="space-y-2">
            {(progress.workouts || []).map((w: any, i: number) => (
              <div key={i} className="flex items-center justify-between rounded-xl bg-slate-50 p-2 text-sm">
