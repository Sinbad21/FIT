      ...
      {days.map((day) => (
        <article key={day.id} className="glass-card rounded-[1.6rem] p-4">
          <div className="flex flex-wrap items-center gap-2">
            <input defaultValue={day.title} onBlur={(e) => patchDay(day.id, { title: e.target.value })} className="min-h-11 flex-1 rounded-xl border border-slate-200 px-3 font-black" />
            ...
            {(day.exercises || []).map((ex: any) => (
              <div key={ex.workoutExerciseId} className="rounded-2xl bg-slate-50 p-3">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                  ...
                  <label>Reps<input defaultValue={ex.reps} onBlur={(e) => patchEx(ex.workoutExerciseId, { reps: e.target.value })} className="mt-1 min-h-10 w-full rounded-lg border border-slate-200 px-2" /></label>
                  <label>Rec. s<input type="number" defaultValue={ex.restSeconds} onBlur={(e) => patchEx(ex.workoutExerciseId, { restSeconds: Number(e.target.value) })} className="mt-1 min-h-10 w-full rounded-lg border border-slate-200 px-2" /></label>
                </div>
              </div>
            ))}
          </div>
          <select onChange={(e) => { addEx(day.id, e.target.value); e.target.value = ''; }} defaultValue="" className="mt-3 min-h-11 w-full rounded-xl border border-dashed border-slate-300 px-3 text-sm">
