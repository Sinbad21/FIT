  ...
  return (
    <div className="space-y-5">
      ...
      <section className="dark-card rounded-[2rem] p-5 text-white shadow-soft">
        <p className="text-sm font-bold text-green-300">Oggi</p>
        <h3 className="mt-1 text-2xl font-black">{d.workoutTitle}</h3>
        <div className="mt-5 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          <div className="rounded-2xl bg-white/10 p-3"><Dumbbell className="mb-2 h-5 w-5 text-green-300" />{d.exercisesCompleted}/{d.exercisesTotal}<br />esercizi</div>
        ...
        <p className="mt-4 rounded-2xl bg-green-400/15 px-4 py-3 text-sm font-bold text-green-100">{d.status}</p>
      </section>
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <MetricCard label="Carbo" value={d.carbsG} target={d.carbsTargetG} unit="g" tone="blue" />
        <MetricCard label="Grassi" value={d.fatG} target={d.fatTargetG} unit="g" tone="gray" />
      </section>
      <WaterTracker current={d.waterL} target={d.waterTargetL} />
      <section className="grid gap-3 sm:grid-cols-2">
        <Link href="/workout" className="rounded-[1.6rem] bg-gray-950 p-5 font-black text-white shadow-soft">Vai all'allenamento di oggi</Link>
        <Link href="/diet" className="rounded-[1.6rem] bg-green-600 p-5 font-black text-white shadow-soft">Conferma i pasti di oggi</Link>
      </section>
    </div>
  );
}
